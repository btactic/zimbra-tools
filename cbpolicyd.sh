...
if [ "x${SERVER_MODE}" = "xYES" ] ; then
  ...
  CREATE USER '${CBPOLICYD_DB_USER}'@'localhost' IDENTIFIED BY '${CBPOLICYD_DB_PASSWORD}';
  GRANT ALL PRIVILEGES ON ${CBPOLICYD_DB_NAME} . * TO '${CBPOLICYD_DB_USER}'@'localhost' WITH GRANT OPTION;
  FLUSH PRIVILEGES ;
EOF
  ...
  DROP USER '${CBPOLICYD_DB_USER}'@'localhost';
